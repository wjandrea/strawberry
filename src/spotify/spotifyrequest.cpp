...
const int SpotifyRequest::kMaxConcurrentArtistsRequests = 1;
const int SpotifyRequest::kMaxConcurrentAlbumsRequests = 1;
const int SpotifyRequest::kMaxConcurrentSongsRequests = 1;
const int SpotifyRequest::kMaxConcurrentArtistAlbumsRequests = 1;
const int SpotifyRequest::kMaxConcurrentAlbumSongsRequests = 1;
const int SpotifyRequest::kMaxConcurrentAlbumCoverRequests = 10;
const int SpotifyRequest::kFlushRequestsDelay = 200;
...
void SpotifyRequest::FlushArtistsRequests() {
  ...
  while (!artists_requests_queue_.isEmpty() && artists_requests_active_ < kMaxConcurrentArtistsRequests) {
    ...
    ParamList parameters = ParamList() << Param("type", "artist");
    if (type_ == QueryType_SearchArtists) {
      parameters << Param("q", search_text_);
    }
    if (request.limit > 0) {
      parameters << Param("limit", QString::number(request.limit));
    }
    if (request.offset > 0) {
      parameters << Param("offset", QString::number(request.offset));
    }
    QNetworkReply *reply = nullptr;
    if (type_ == QueryType_Artists) {
      reply = CreateRequest("me/following", parameters);
    }
    if (type_ == QueryType_SearchArtists) {
...
void SpotifyRequest::ArtistsReplyReceived(QNetworkReply *reply, const int limit_requested, const int offset_requested) {
  ...
  int offset = 0;
  if (obj_artists.contains("offset")) {
    offset = obj_artists["offset"].toInt();
  }
...
void SpotifyRequest::AlbumsReceived(QNetworkReply *reply, const Artist &artist_artist, const int limit_requested, const int offset_requested) {
  ...
  for (const QJsonValueRef value_item : array_items) {
    ...
    if (obj_item.contains("album")) {
      QJsonValue json_item = obj_item["album"];
      if (!json_item.isObject()) {
        Error("Invalid Json reply, album in array is not a object.", json_item);
        continue;
      }
      obj_item = json_item.toObject();
    }
    ...
    if (!obj_item.contains("id")) {
      Error("Invalid Json reply, item is missing ID.", obj_item);
      continue;
    }
    if (!obj_item.contains("name")) {
      Error("Invalid Json reply, item is missing name.", obj_item);
      continue;
    }
    if (!obj_item.contains("images")) {
      Error("Invalid Json reply, item is missing images.", obj_item);
      continue;
    }
    album.album_id = obj_item["id"].toString();
    album.album = obj_item["name"].toString();

    if (artist_artist.artist_id.isEmpty() && obj_item.contains("artists") && obj_item["artists"].isArray()) {
      QJsonArray array_artists = obj_item["artists"].toArray();
      for (const QJsonValueRef value : array_artists) {
        if (!value.isObject()) {
          continue;
        }
        QJsonObject obj_artist = value.toObject();
        if (obj_artist.isEmpty() || !obj_artist.contains("id") || !obj_artist.contains("name")) continue;
        artist.artist_id = obj_artist["id"].toString();
        artist.artist = obj_artist["name"].toString();
        break;
      }
    }
    else {
    ...
    if (obj_item.contains("images") && obj_item["images"].isArray()) {
      ...
      for (const QJsonValueRef value : array_images) {
        ...
        album.cover_url = QUrl(obj_image["url"].toString());
      }
    }

    if (obj_item.contains("tracks") && obj_item["tracks"].isObject()) {
      QJsonObject obj_tracks = obj_item["tracks"].toObject();
      if (obj_tracks.contains("items") && obj_tracks["items"].isArray()) {
        QJsonArray array_tracks = obj_tracks["items"].toArray();
...
void SpotifyRequest::SongsReceived(QNetworkReply *reply, const Artist &artist, const Album &album, const int limit_requested, const int offset_requested) {
  ...
  for (const QJsonValueRef value_item : array_items) {
    ...
    if (obj_item.contains("item") && obj_item["item"].isObject()) {
      obj_item = obj_item["item"].toObject();
    }

    if (obj_item.contains("track") && obj_item["track"].isObject()) {
      obj_item = obj_item["track"].toObject();
    }
...
void SpotifyRequest::ParseSong(Song &song, const QJsonObject &json_obj, const Artist &album_artist, const Album &album) {
  ...
  qint64 duration = json_obj["duration_ms"].toVariant().toLongLong() * kNsecPerMsec;
